...
menuRouter.post('/', authenticate, authorize('OWNER', 'ADMIN'), upload.single('image'), async (request: AuthenticatedRequest, response, next) => {
  try {
...
menuRouter.put('/:menuId', authenticate, authorize('OWNER', 'ADMIN'), upload.single('image'), async (request: AuthenticatedRequest, response, next) => {
  try {
